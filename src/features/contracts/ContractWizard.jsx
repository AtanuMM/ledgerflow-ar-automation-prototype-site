...
function ContractWizard({ setPage, notify }) {
  ...
  const [form, setForm] = useState({ title: "Retail commerce transformation", customer: "Capital Retail Solutions", country: "India", state: "Delhi", entity: "Matrix Media — India (West Bengal)", model: "Project" });
  ...
  return <><button onClick={() => setPage("contracts")} className="btn-ghost mb-4 !px-0"><ArrowLeft size={17} />Back to contracts</button><PageHead eyebrow={`New contract · Step ${step + 1} of 5`} title={stepNames[step]} desc="Capture the commercial agreement once. LedgerFlow will carry it through every invoice." />
    <Stepper step={step} setStep={setStep} /><div className="panel p-5 sm:p-7"><div className="mx-auto max-w-5xl">{step === 0 && <BasicStep form={form} setForm={setForm} />}{step === 1 && <ModelStep form={form} setForm={setForm} />}{step === 2 && <CommercialStep form={form} setForm={setForm} />}{step === 3 && <TaxStep form={form} />}{step === 4 && <ReviewStep form={form} />}</div></div>
    <div className="sticky bottom-4 mt-5 flex items-center justify-between rounded-2xl border bg-white/90 p-3 shadow-xl shadow-slate-200/70 backdrop-blur"><button disabled={step === 0} onClick={() => setStep(step - 1)} className="btn-secondary disabled:opacity-40"><ArrowLeft size={16} />Back</button><div className="hidden text-xs text-slate-400 sm:block">Changes are saved automatically</div><button onClick={next} className="btn-primary">{step === 4 ? "Submit for approval" : "Save & continue"} {step === 4 ? <CheckCircle2 size={16} /> : <ArrowRight size={16} />}</button></div></>;
}
